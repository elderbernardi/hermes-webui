...
**Shared-helper home.** `_acervo_root`, `_read_frontmatter_meta`, `_ACERVO_NATURES` and peers move to `acervo_tab.py` as the single home. **Refinement locked at planning (2026-07-13):** the fork-owned Explorer/Studio modules consume 20 `routes.<name>` attributes — rather than retargeting their imports (which would edit 9 proven files), `routes.py` **re-exports** the moved names (`from api.acervo_tab import …` at file bottom), so the fork-owned files stay **byte-identical**. Core-route helpers (`j`, `bad`, session resolution, folder-zip) stay in `routes.py`; `acervo_tab.py` reaches them via the deferred `import api.routes as routes` pattern already proven in `acervo_explorer.py`. Module boundary: "acervo-specific logic → `acervo_tab.py`"; net `routes.py` acervo surface ≈ 12 lines (2 dispatch delegations + 1 re-export block).
