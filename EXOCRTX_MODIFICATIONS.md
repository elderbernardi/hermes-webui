...
### MOD-014: Sala viva (F3) — camada observe-and-translate in-process

- **Resumo:** adiciona uma camada **observe-and-translate** que reflete a sessão Hermes **lançada** de volta no Canvas/Cockpit — artefatos, clarify, kanban, traces do loop e Draft-First viram cards verificáveis `sala_*`, sem **nunca** tocar o runtime da sessão. Fonte durável única na v1 = `_tasks/<task_id>/conduct.jsonl` (trilha append-only escrita pelo AGENTE VIVO sob as skills `excrtx-conduct-*`, tudo DECLARADO por tool-write ⇒ anti-narração por construção, OD-1) + as duas filas HITL multi-subscriber que ela **observa** (`clarify.sse_subscribe`, `route_approvals._approval_sse_subscribe`). Registro/transporte **próprio** (`SALA_ROOMS` ≠ `CANVAS_JOBS`/`CURADOR_ROOMS`) + SSE não-fechante, herdando as costuras já provadas do F2 (MOD-013). Arquivos novos: `api/sala_reducer.py` (núcleo PURO IO-free: `SalaState.ingest(frame) -> [(event, payload)]`, máquina de fases `PHASE_TO_COLUMN`, 3 contadores de bound, ordem de autoridade — a costura de teste hermética), `api/canvas_sala.py` (casca impura: `SALA_ROOMS`+`_room`/`_emit`/`_stream_events` clonados do Curador; índice reverso `_LAUNCHED`+`register_launch`/`resolve`/`_rebuild_launched`; observador daemon por sessão poll→reducer→emit; pontes clarify/approval; `handle_sala_get/post`; gate `SALA_ENABLE` default off), `static/canvas-sala.js` (ilha vanilla clonada de `canvas-curador.js`: `#cvt-sala-zone` própria, 3ª `EventSource` em `/api/canvas/sala/stream`, 1 renderer por evento `sala_*`, aceitar→`window.CVT.acceptOps`, respostas HITL via `/api/clarify/respond`+`/api/approval/respond`, AUTH-words capturadas in-island → `/api/canvas/patch`).
- **Tipo:** backend (novos endpoints `/api/canvas/sala/{stream,state,observe}`, despachados por **forward**, não por hook em `routes.py`) + frontend (ilha vanilla JS, sem build) + testes + 2 skills de condução no exocortex + ADR-CT-07. **Feature entregue** (F3 do épico Canvas de Tarefas).
- **Os 10 eventos `sala_*`:** `sala_phase` (chip de fase, nunca token) · `sala_artifact` (`ops`→`/artifacts/expected/-`) · `sala_gap` (clarify observado ou 2ª busca vazia; `ops`→`/gaps/-`) · `sala_kanban` (projeção fase→coluna, display-only) · `sala_next_move` (`ops`→`/next_moves/-`) · `sala_trace` (intent/twins/pending; `ops?`→`/assumptions/-`|`/scope/-`) · `sala_draft` (Draft-First EX-08 ou gate de tool-permission) · `sala_auth` (humano aprova digitando as palavras verbatim; `ops`→`/authorization/-`) · `sala_interrupt` (3º verify-fail; `ops`→`/gaps/-`) · `sala_finding` (surprise code×check×spec; `authority=[executivo,spec,tests,codigo]`, não-bloqueante).
- **Forward em vez de hook em `routes.py`:** mesma tática do MOD-013 — o F1b já esgotou o teto de 8 linhas; a Sala despacha `/api/canvas/sala/*` por **forward** no topo de `handle_canvas_get`/`handle_canvas_post` ⇒ **`routes.py` fica em 0 linhas novas**. Edição em `api/canvas_tarefas.py` também espelha a whitelist de patch (`_WHITELIST_RAW` ganha `/authorization/*` — `authorization[]` já canônico no `_MINIMAL`/template harness, ADR-CT-06 §5, só faltava ser editável) e o `_handle_launch` grava a linkagem `register_launch` + `_tasks/<canvas_id>/launch.yaml`.
- **Guardrails:** a camada **não cunha primitiva bloqueante própria** — só observa `clarify.sse_subscribe` e `route_approvals._approval_sse_subscribe` (ambos multi-subscriber; um teste assegura **zero** `register_gateway_notify`); o único bloqueio HITL é o agente vivo auto-invocando clarify/approval (ou terminando o turno em Draft-First, EX-08). Logo "interrupções só nas 3 classes" é **estrutural**. O "sticky/long-timeout" do bound-interrupt é **comportamental+watchdog** (`clarify._with_timeout_metadata` sempre seta `expires_at`), não estrutural. Toda a camada é gated por `SALA_ENABLE` (default off). O backstop de artefatos via `run_journal` (para artefatos que o agente esquece de declarar) fica para **F5** (o shape dict-return/`payload`-nested/`run_id:seq`-cursor precisa de redesenho de bootstrap; a v1 confia no `conduct.jsonl`).
- **Propagação SOUL (C-E):** o método de condução chega à sessão lançada pela cadeia `skill compiled_rules → compile_soul.py → SOUL_SEED.md → (setup.sh step-07 cp) → $HERMES_HOME/SOUL.md → run_agent.load_soul_md()` (persona da sessão lançada = slot #1 `SOUL.md`, `personality=None`). Forçar `profile=` no launch é **no-op no smoke isolado** (default profile já mapeia para `$HERMES_HOME`) ⇒ F3 **não** força profile; escopagem vetor→profile fica para F5.
- **Relação com MOD-011/012/013:** construído **sobre** o F1b (MOD-012) + F2 (MOD-013) já mergeados em `exocortex/stable@14d880b6`, na branch `collab/canvas-f3`; reusa `TaskStore`/schema v0.5/whitelist/costuras de room+SSE sem alterar comportamento existente (o forward da Sala roda ao lado do forward do Curador, sem tocar a lógica de `draft`/`get`/`list`/`job`/`stream`/`patch`/`brief`/`launch` nem a do Curador).
- **Rebase-safety:** comportamento novo majoritariamente em arquivos novos (`api/sala_reducer.py`, `api/canvas_sala.py`, `static/canvas-sala.js`); edições aditivas em arquivos já fork-owned (`api/canvas_tarefas.py` forward+whitelist+launch-link, `static/canvas-tarefas.js` 1 linha guardada de handoff, `static/canvas-dev.html` 1 `<script>`, `static/canvas-tarefas.css` classes `.cvt-sala-*` append-only). `api/routes.py`/`index.html`/`ui.js`/`messages.js`/`sessions.js`/`panels.js`/`boot.js`/`style.css` **byte-untouched**.
